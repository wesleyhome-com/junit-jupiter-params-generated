...
class ParametersGenerator(
    ...
    private val options: List<List<Any?>> = testModel.testParameters.mapIndexed { index, testParameter: TestParameter ->
        dataProviders.filter { it.providesDataFor(testParameter) }
            .flatMap { it.createParameterOptionsData(testParameter) }
            .also {
                if(it.isEmpty()) {
